<template>
  <div :class="wrapStyleClasses">
    ...
    <div class="vgt-inner-wrap" :class="{ 'is-loading': isLoading }">
      ...
      <div
        v-if="(selectedCount || selectedRowCount) && !disableSelectInfo"
          ...
          >
            <template #table-column="slotProps">
              <slot name="table-column" :column="slotProps.column">
                <span>{{ t(slotProps.column.label) }}</span>
              </slot>
            </template>
        ...
        <table id="vgt-table" ref="table" :class="tableStyles">
          ...
          <tbody v-for="(headerRow, hIndex) in paginated" :key="hIndex">
            ...
            <template v-for="(row, index) in headerRow.children">
              <tr
                v-if="
                  (groupOptions.collapsable ? headerRow.vgtIsExpanded : true) &&
...
import { ObjectReader, t } from "~/services";
...
export default {
  ...
  computed: {
    ...
    showEmptySlot() {
      ...
      let deleted = 0
      this.paginated[0].children.forEach((item) => {
        if (item.deleted === true) {
          deleted++
        }
      })

      if (deleted === this.paginated[0].children.length) {
        return true
      }
    ...
    selectionInfo() {
      return `${this.selectedCount || this.selectedRowCount} ${this.selectionText}`
    },
  ...
  methods: {
    ...
    onCheckboxClicked(row, index, event) {
      ...
      if (row['vgtSelected'] === false) {
        this.$emit('un-selected-row', row)
      }
    ...
    collect(obj, field) {
      ...
      function dig(obj, selector) {
        return ObjectReader(obj, selector)
      }
