...
<script setup lang="ts">
  ...
  import { Flatpickr, Notyf, t } from '~/services'
  ...
  onMounted(() => {
    setTimeout(() => {
      if (props.default === 'inherit') {
        temp.value_ = moment(temp.value, props.input).format(props.output)
      } else if (props.default === 'now') {
        temp.value_ = moment(new Date(), props.input).format(props.output)
      } else {
        temp.value_ = props.default
      }

      init.value = Flatpickr(`.flatpickr-${props.name}-time`, {
        enableTime: true,
        noCalendar: true,
