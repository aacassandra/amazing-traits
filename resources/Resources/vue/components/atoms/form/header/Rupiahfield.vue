...
<script setup lang="ts">
  ...
  if (!temp.value) {
    ...
    temp.value_ = IntToRupiah(temp.value.toString(), 'Rp. ')
  }
  ...
  const methods = {
    onChange: (evt) => {
      if (evt.type === 'focus') {
        temp.value_ = RupiahToInt(evt.target.value || 'Rp. 0') || ''
      } else if (evt.type === 'blur') {
        temp.value_ =
          IntToRupiah(evt.target.value.toString(), 'Rp. ') || 'Rp. 0'
      }

      temp.value = RupiahFixValue(evt.target.value)
  ...
  watch(
    ...
    () => {
      if (!nativeEdit.value && temp.value !== props.outVal) {
        ...
        temp.value_ = IntToRupiah(props.outVal.toString(), 'Rp. ')
      }
    },
  )
