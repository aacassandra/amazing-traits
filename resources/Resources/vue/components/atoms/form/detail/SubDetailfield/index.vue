<template>
  ...
  <Modal
    v-model="showModal"
    :width="editor.options.modal?.width"
  ...
  >
    ...
    <template #modal-body>
      <SubDetailGenerator
        ...
        :disabled="editor.options.disabled || isDisabled"
      />
    </template>
...
<script setup lang="ts">
  ...
  const props = defineProps({
    // [FIXME]: default not available, need check
    rowIndex: {
      type: Number,
      required: true,
    },
  ...
  const tempVal = ref(
    computed({
      get: () =>
        ObjectReader(temp.form.rows[props.rowIndex], props.field) || [],
      set: (val) => {
  ...
  watch(tempVal, () => {
    if (tempVal.value && !nativeEdit.value) {
      const newVal = tempVal.value || []
      backupRows.value = JSON.stringify(newVal)
      if (newVal.length) {
        methods.onChange({ type: 'change' })
      }
    }
