      ...
      <template #card-header>
        <div class="flex">
          ...
          <div
            v-if="
              CheckPermissions(
                ['cluster-master-securities-create'],
                permissions,
              )
            "
            class="fle-grow-0"
...
<script setup lang="ts">
  ...
  const pageConfigs = reactive<PageConfigs>({
    ...
    title: 'menu.securities',
    model: table_prefix + 'm_securities',
    formRoute: 'cluster-master-securities-form',
  })
  ...
  const data = reactive<DataLanding>({
    ...
    api: {
      ...
      overrideParams: (oldParams) => {
        // oldParams['wherejsoncontainsin'] = 'roles=["SECURITY"]'
    ...
    actions: {
      preview: () => {
        if (
          !CheckPermissions(
            ['cluster-master-securities-preview'],
            permissions.value,
          )
        ) {
          Notyf({
      ...
      edit: () => {
        if (
          !CheckPermissions(
            ['cluster-master-securities-edit'],
            permissions.value,
          )
        ) {
          Notyf({
      ...
      delete: () => {
        if (
          !CheckPermissions(
            ['cluster-master-securities-delete'],
            permissions.value,
          )
        ) {
          Notyf({
