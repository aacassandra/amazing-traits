    ...
    <Card>
      <template #card-header>
        <div class="flex">
          ...
          <div
            v-if="
              CheckPermissions(['cluster-master-incentives-create'], permissions)
            "
            class="fle-grow-0"
          >
...
<script setup lang="ts">
  ...
  const pageConfigs = reactive<PageConfigs>({
    ...
    title: 'menu.general',
    model: table_prefix + 'm_incentives',
    formRoute: 'cluster-master-incentives-form',
  })
  ...
  const data = reactive<DataLanding>({
    ...
    actions: {
      preview: () => {
        if (
          !CheckPermissions(
            ['cluster-master-incentives-preview'],
            permissions.value,
          )
        ) {
          Notyf({
      ...
      edit: () => {
        if (
          !CheckPermissions(['cluster-master-incentives-edit'], permissions.value)
        ) {
          Notyf({
            type: 'warning',
            message: "You don't have permission",
      ...
      delete: () => {
        if (
          !CheckPermissions(
            ['cluster-master-incentives-delete'],
            permissions.value,
          )
        ) {
          Notyf({
