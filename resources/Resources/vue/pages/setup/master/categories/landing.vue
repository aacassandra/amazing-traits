<template>
      ...
      <Card>
        <template #card-header>
          <div class="flex">
            ...
            <div
              v-if="
                CheckPermissions(['setup-master-packages-create'], permissions)
              "
              class="fle-grow-0"
            >
  ...
  <script setup lang="ts">
    ...
    const pageConfigs = reactive<PageConfigs>({
      ...
      model: table_prefix + 'm_categories',
      formRoute: 'setup-master-categories-form',
    })
    ...
    const data = reactive<DataLanding>({
      ...
      actions: {
        preview: () => {
          if (
            !CheckPermissions(
              ['setup-master-categories-preview'],
              permissions.value,
            )
          ) {
            Notyf({
        ...
        edit: () => {
          if (
            !CheckPermissions(['setup-master-packages-edit'], permissions.value)
          ) {
            Notyf({
              type: 'warning',
              message: "You don't have permission",
        ...
        delete: () => {
          if (
            !CheckPermissions(['setup-master-packages-delete'], permissions.value)
          ) {
            Notyf({
              type: 'warning',
              message: "You don't have permission",
