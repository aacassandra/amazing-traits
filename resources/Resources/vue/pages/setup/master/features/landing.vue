    ...
    <Card>
      <template #card-header>
        <div class="flex">
          ...
          <div
            v-if="
              CheckPermissions(['setup-master-features-create'], permissions)
            "
            class="fle-grow-0"
          >
...
<script setup lang="ts">
  ...
  const pageConfigs = reactive<PageConfigs>({
    ...
    title: 'menu.features',
    model: table_prefix + 'm_features',
    formRoute: 'setup-master-features-form',
  })
  ...
  const data = reactive<DataLanding>({
    ...
    actions: {
      ...
      edit: () => {
        if (
          !CheckPermissions(['setup-master-features-edit'], permissions.value)
        ) {
          Notyf({
            type: 'warning',
            message: "You don't have permission",
      ...
      delete: () => {
        if (
          !CheckPermissions(['setup-master-features-delete'], permissions.value)
        ) {
          Notyf({
            type: 'warning',
            message: "You don't have permission",
