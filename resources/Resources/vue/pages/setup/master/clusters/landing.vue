    ...
    <Card>
      <template #card-header>
        <div class="flex">
          ...
          <div
            v-if="
              CheckPermissions(['setup-master-general-create'], permissions)
            "
            class="fle-grow-0"
          >
...
<script setup lang="ts">
  ...
  const pageConfigs = reactive<PageConfigs>({
    ...
    formRoute: 'setup-master-clusters-form',
  })
  ...
  const data = reactive<DataLanding>({
    ...
    api: {
      ...
      root: '',
      parameters: {
        where: "ihm_m_clusters.status='VERIFIED'",
      },
    },
    columns: [
      {
    ...
    actions: {
      preview: () => {
        if (
          !CheckPermissions(['setup-master-general-preview'], permissions.value)
        ) {
          Notyf({
            type: 'warning',
      ...
      edit: () => {
        if (
          !CheckPermissions(['setup-master-general-edit'], permissions.value)
        ) {
          Notyf({
            type: 'warning',
            message: "You don't have permission",
      ...
      delete: () => {
        if (
          !CheckPermissions(['setup-master-general-delete'], permissions.value)
        ) {
          Notyf({
            type: 'warning',
            message: "You don't have permission",
