...
<script setup lang="ts">
  ...
  const landingRoute = 'setup-master-epic'
        ...
        properties: {
          feature: {
            ...
            listener: (element, type, value) => {
              if (type === 'change') {
                if (!isMounted.value && schema.mode !== 'create') {
                  isMounted.value = true
                  return
                }

                if (!value) {
                  element.stack_category_id.setValue('')
          ...
          stack_category_id: {
            ...
            options: {
              ...
              api: {
                ...
                parameters: {
                  paginate: 25,
                  where: "`group`='STACK CATEGORY' AND value_2 <> ''",
                },
                cache: false,
              },
            },
            listener: (element, type, value) => {
              if (type == 'change') {
                const { feature, stack_category_id, key } = element
                const all = stack_category_id.getValueFull()
                if (feature.value && value) {
                  key.setValue(`${all.value_2} - ${feature.value}`)
                } else {
                  key.setValue(``)
                }
              }
