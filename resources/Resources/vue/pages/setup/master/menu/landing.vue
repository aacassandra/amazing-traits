    ...
    <Card>
      <template #card-header>
        <div class="flex">
          ...
          <div
            v-if="CheckPermissions(['setup-master-menu-create'], permissions)"
            class="fle-grow-0"
          >
...
<script setup lang="ts">
  ...
  const pageConfigs = reactive<PageConfigs>({
    ...
    formRoute: 'setup-master-menu-form',
  })
  ...
  const data = reactive<DataLanding>({
    ...
    actions: {
      preview: () => {
        if (
          !CheckPermissions(['setup-master-menu-preview'], permissions.value)
        ) {
          Notyf({
            type: 'warning',
      ...
      edit: () => {
        if (!CheckPermissions(['setup-master-menu-edit'], permissions.value)) {
          Notyf({
            type: 'warning',
            message: "You don't have permission",
            duration: 3000,
      ...
      delete: () => {
        if (
          !CheckPermissions(['setup-master-menu-delete'], permissions.value)
        ) {
          Notyf({
            type: 'warning',
            message: "You don't have permission",
