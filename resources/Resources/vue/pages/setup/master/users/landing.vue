    ...
    <Card>
      <template #card-header>
        <div class="flex">
          ...
          <div
            v-if="CheckPermissions(['setup-master-users-create'], permissions)"
            class="fle-grow-0"
          >
...
<script setup lang="ts">
  ...
  const pageConfigs = reactive<PageConfigs>({
    ...
    formRoute: 'setup-master-users-form',
  })
  ...
  const data = reactive<DataLanding>({
    ...
    api: {
      ...
      overrideParams: (oldParams) => {
        if (!['SUPERADMIN'].includes(authStore.user.role)) {
          oldParams['wherejsoncontainsin'] =
            'roles=["ADMIN", "RT", "RW", "STAFF", "SECURITY"]'
        }
    ...
    actions: {
      preview: () => {
        if (
          !CheckPermissions(['setup-master-users-preview'], permissions.value)
        ) {
          Notyf({
            type: 'warning',
      ...
      edit: () => {
        if (!CheckPermissions(['setup-master-users-edit'], permissions.value)) {
          Notyf({
            type: 'warning',
            message: "You don't have permission",
            duration: 3000,
      ...
      delete: () => {
        if (
          !CheckPermissions(['setup-master-users-delete'], permissions.value)
        ) {
          Notyf({
            type: 'warning',
            message: "You don't have permission",
